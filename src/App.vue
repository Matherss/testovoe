 ...
 Season(_id: 5099) {
  calendar(offset:0, limit:${this.limit}) {
    total, items {
      ...
      .then((result) => {
        ...
        const teamsmap = this.info.data.Season.calendar.items.reduce((acc: object, game: any) => {
  ...
  get filters() {
    ...
    if (this.checkedNames) {
      ...
      return items
        .filter(
          (item: { teamHome: { name: string }; teamAway: { name: string } }) =>
            item.teamHome.name.match(regex) || item.teamAway.name.match(regex)
        )
        .filter(
          (item: { teamHome: { name: string }; teamAway: { name: string } }) =>
            item.teamHome.name.match(regex2) || item.teamAway.name.match(regex2)
        );
    }
    return items;
  }
